...
\author[
  ...
  noteref={n1},
  corref={aff1},                 
  email={quangpmnguyen@gmail.com}   % email address
...
\note[id=n1]{Co-corresponding authors}
\note[id=n2]{These authors jointly supervised this work}
\end{artnotes}
...
\parttitle{Background} In traditional genomic studies, pathway analysis is a powerful tool that allows researchers to summarize complex genomic variables in the form of biologically informative sets. Since the taxonomic composition of the human microbiome is highly variable between individuals, applying enrichment methods using functionally driven taxon sets can increase both the reproducibility and interpretability of microbiome association studies. However, there is still an open question of how to construct such sets. Here, we evaluate microbial trait databases, which aggregates experimentally determined microbial phenotypes, as a potential avenue for meaningful construction of taxon sets.  

\parttitle{Methods} Using publicly available microbiome sequencing data sets (both metabarcoding - using the 16S rRNA gene - and whole-genome metagenomics), we assessed these trait-based sets on two aspects: first, do they cover the diversity of microbes obtained from a typical data set, and second, do they confer additional predictive power on disease prediction tasks when assessed against measured pathway abundances and PICRUSt2 prediction.   

\parttitle{Results} Trait annotations are well annotated to a small number but most abundant taxa within the community, concordant with the concept of the core-peripheral microbiome. This pattern is consistent across all categories of traits and body-sites for whole genome sequencing data, but much more heterogenous and inconsistent in metabarcoding data due to difficulties in assigning species-level traits to genus. However, trait-set features are well predictive of disease outcomes compared against predicted and measured pathway abundances. Most important trait-set features are more interpreable and reveal interesting insights on the relationship between microbiome, its function, and health outcomes. 

\parttitle{Conclusion} We demonstrated that trait-based taxonomic sets are useful resources to explore mechanistically driven hypotheses in microbiome data analysis.  
...
For IBD, we utilized data from the integrative HMP \cite{proctor2019integrative}, Gevers et al. \cite{gevers2014treatmentnaive}, Hall et al. \cite{hall2017novel}, Ijaz et al. \cite{ijaz2017distinct}, Li et al. \cite{li2014integrated}, Nielsen et al. \cite{nielsen2014identificationa}, and Vich Vila et al. \cite{vichvila2018gut}.   
...
All data for this manuscript is already publicly available. Metagenomic data are available via the R package \texttt{curatedMetagenomicData}. A list of studies and associated PMIDs are available on the package website (https://waldronlab.io/curatedMetagenomicData/articles/available-studies.html). Processed 16S rRNA gene sequencing data from the Human Microbiome Project was obtained from \texttt{HMP16SData}, raw data an be found at (https://www.hmpdacc.org/hmp/HMQCP/). Other raw data sets are available from the European Nucleotide Archive at accession numbers PRJEB13679 and PRJEB6070. Reproducible scripts are publicly archived on Zenodo at (https://doi.org/10.5281/zenodo.7805921).  
...
\bibliography{microbe_trait}      % Bibliography file (usually '*.bib' )
